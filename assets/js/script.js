...
// Create a task card
function createTaskCard({ id, taskTitle, taskDueDate, taskDescription, status }, isDone) {
  let statusClass = '';
  if (status === 'to-do') {
    statusClass = 'bg-primary'; // Blue for To Do if it's not due tomorrow or today
  } else if (status === 'in-progress') {
    statusClass = 'bg-success'; // Green for In Progress if it's not due tomorrow or today
  }

  const newTaskCard = $(
    `<div class='card task-card draggable ${statusClass} ${!isDone && compareDates(taskDueDate).cardBg}'
    data-task='${id}'>`
  );
